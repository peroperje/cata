...
class GroqProvider(AIBaseProvider):
    async def process(self, cv_text: str, form_data: List[dict], api_key: str, model_name: str, instruction: str = None, job_description: str = None) -> dict:
        ...
        try:
            ...
            print(f"DEBUG: Error calling Groq API: {str(e)}")
        ...
        try:
            ...
            if "mappings" in data and isinstance(data["mappings"], list):
                ...
                raise ValueError("Groq response missing 'mappings' key.")
                
        except json.JSONDecodeError:
            raise ValueError("Groq returned invalid JSON structure.")

    async def extract_metadata(self, page_text: str, api_key: str, model_name: str) -> dict:
        print(f"DEBUG: Extracting metadata with Groq model {model_name}")
        client = AsyncGroq(api_key=api_key)
